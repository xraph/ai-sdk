...
// ToolRun represents a tool that was executed.
// Renamed from ToolExecution for clarity.
type ToolRun struct {
	Name      string
...
// ToolExecution is an alias for ToolRun for backward compatibility.
// Deprecated: Use ToolRun instead.
type ToolExecution = ToolRun
...
func (a *Agent) Execute(ctx context.Context, input string) (*AgentResponse, error) {
	...
	if a.logger != nil {
		a.logger.Info("Agent executing",
			...
			logger.String("session_id", a.state.SessionID),
			logger.Int("input_length", len(input)),
		)
	}
	...
	if err := a.SaveState(ctx); err != nil {
		if a.logger != nil {
			a.logger.Warn("Failed to save agent state", logger.String("error", err.Error()))
		}
	}
...
func (a *Agent) executeTool(ctx context.Context, toolCall ToolCallResult) ToolExecution {
	...
	if a.metrics != nil {
		a.metrics.Counter("forge.ai.sdk.agent.tool_calls", metrics.WithLabel("tool", toolCall.Name)).Inc()
		a.metrics.Histogram("forge.ai.sdk.agent.tool_duration", metrics.WithLabel("tool", toolCall.Name)).Observe(execution.Duration.Seconds())
	}
...
func (a *Agent) executeToolsParallel(ctx context.Context, toolCalls []ToolCallResult) []ToolExecution {
	...
	if a.logger != nil && len(toolCalls) > 1 {
		a.logger.Debug("Executed tools in parallel",
			logger.String("agent_id", a.ID),
			logger.Int("tool_count", len(toolCalls)),
		)
	}
...
func (a *Agent) LoadState(ctx context.Context, sessionID string) error {
	...
	if a.logger != nil {
		a.logger.Info("Agent state loaded",
			...
			logger.Int("version", state.Version),
		)
	}
