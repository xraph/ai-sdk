...
func NewArtifactRegistry(logger logger.Logger, metrics metrics.Metrics) *ArtifactRegistry {
...
func (r *ArtifactRegistry) Create(artifact *Artifact) error {
	...
	if r.metrics != nil {
		r.metrics.Counter("forge.ai.sdk.artifacts.created",
			metrics.WithLabel("name", artifact.Name),
			metrics.WithLabel("type", string(artifact.Type)),
		).Inc()
	}
...
func (r *ArtifactRegistry) Update(id string, content string) error {
	...
	if r.metrics != nil {
		r.metrics.Counter("forge.ai.sdk.artifacts.updated",
			metrics.WithLabel("name", artifact.Name),
			metrics.WithLabel("type", string(artifact.Type)),
		).Inc()
	}
...
func (r *ArtifactRegistry) Delete(id string) error {
	...
	if r.metrics != nil {
		r.metrics.Counter("forge.ai.sdk.artifacts.deleted",
			metrics.WithLabel("id", id),
		).Inc()
	}
