...
func NewCostTracker(logger logger.Logger, metrics metrics.Metrics, opts *CostTrackerOptions) *CostTracker {
	ct := &CostTracker{
		logger:  logger,
		metrics: metrics,
...
func (ct *CostTracker) RecordUsage(ctx context.Context, usage UsageRecord) error {
	...
	if ct.metrics != nil {
		ct.metrics.Counter("forge.ai.sdk.cost.requests",
			metrics.WithLabel("provider", usage.Provider),
			metrics.WithLabel("model", usage.Model),
		).Inc()
