...
func ExampleSSEHandler(log logger.Logger) func(w interface{ Write([]byte) (int, error) }, ctx context.Context) {
...
func FullIntegrationExample(ctx context.Context, log logger.Logger, met metrics.Metrics) error {
	// Create registries
	uiRegistry := sdk.NewUIToolRegistry(log, met)
	...
	result, err := uiRegistry.ExecuteUITool(
		...
		func(event llm.ClientStreamEvent) error {
			log.Debug("SSE Event",
				logger.F("type", event.Type),
				logger.F("part_id", event.PartID),
				logger.F("section", event.Section),
			)
			return nil
		},
		log,
	...
	log.Info("UI Tool execution completed",
		logger.F("tool", result.ToolName),
