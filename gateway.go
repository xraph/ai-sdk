...
// StreamingLLMManager is an interface that extends LLMManager with streaming capabilities.
// It's used by the AIGateway for providers that support streaming.
type StreamingLLMManager = LLMManager
...
func NewAIGateway(logger logger.Logger, metrics metrics.Metrics) *AIGateway {
	return &AIGateway{
