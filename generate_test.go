...
func TestStringHelpers(t *testing.T) {
	// Test strings.Index (replaced indexOf)
	if idx := strings.Index("hello world", "world"); idx != 6 {
		t.Errorf("expected index 6, got %d", idx)
	}

	if idx := strings.Index("hello world", "missing"); idx != -1 {
		t.Errorf("expected index -1, got %d", idx)
	}

	// Test strings.Replace (replaced replaceFirst)
	result := strings.Replace("hello world", "world", "universe", 1)
	if result != "hello universe" {
		t.Errorf("expected 'hello universe', got '%s'", result)
	}

	// Test strings.ReplaceAll (replaced replaceAll)
	result = strings.ReplaceAll("hello world", "world", "universe")
	if result != "hello universe" {
		t.Errorf("expected 'hello universe', got '%s'", result)
	}
}
