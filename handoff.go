...
func NewAgentRegistry(logger logger.Logger, metrics metrics.Metrics) *AgentRegistry {
...
func (r *AgentRegistry) Register(agent *Agent) error {
	...
	if r.metrics != nil {
		r.metrics.Counter("ai-sdk.registry.agents_registered").Inc()
		r.metrics.Gauge("ai-sdk.registry.agents_count").Set(float64(len(r.agents)))
	}
...
func (r *AgentRegistry) Unregister(agentID string) error {
	...
	if r.logger != nil {
		r.logger.Info("Agent unregistered", logger.String("agent_id", agentID))
	}

	if r.metrics != nil {
		r.metrics.Counter("ai-sdk.registry.agents_unregistered").Inc()
		r.metrics.Gauge("ai-sdk.registry.agents_count").Set(float64(len(r.agents)))
	}
...
func (r *DefaultAgentRouter) findBestMatch(input string, agents []*Agent) *Agent {
	...
	for _, agent := range agents {
		...
		if strings.Contains(inputLower, strings.ToLower(agent.Name)) {
			score += 10
		}

		// Check if any tool is mentioned
		for _, tool := range agent.tools {
			if strings.Contains(inputLower, strings.ToLower(tool.Name)) {
				score += 5
			}

			if strings.Contains(inputLower, strings.ToLower(tool.Description)) {
				score += 2
			}
		}
...
func (m *HandoffManager) Handoff(ctx context.Context, handoff *AgentHandoff) (*HandoffResult, error) {
	...
	if m.logger != nil {
		m.logger.Info("Agent handoff completed",
			logger.String("from_agent", handoff.FromAgentID),
			logger.String("to_agent", handoff.ToAgentID),
...
// containsAnyKeyword checks if input contains any keyword from the description.
func containsAnyKeyword(input, description string) bool {
	...
	for _, word := range words {
		...
		if len(word) > 3 && strings.Contains(input, word) {
			return true
		}
	}
