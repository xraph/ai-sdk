...
func NewImageBuilder(ctx context.Context, generator ImageGenerator, logger logger.Logger, metrics metrics.Metrics) *ImageBuilder {
	return &ImageBuilder{
		ctx:            ctx,
		generator:      generator,
		logger:         logger,
...
func (b *ImageBuilder) Generate() (*ImageResponse, error) {
	...
	if err != nil {
		...
		if b.metrics != nil {
			b.metrics.Counter("forge.ai.sdk.image.errors", metrics.WithLabel("model", b.model)).Inc()
		}
	...
	if b.logger != nil {
		b.logger.Info("Image generation completed",
			...
			logger.Int("count", len(response.Data)),
			logger.Duration("duration", duration),
		)
	}

	if b.metrics != nil {
		b.metrics.Counter("forge.ai.sdk.image.success", metrics.WithLabel("model", b.model)).Inc()
		b.metrics.Histogram("forge.ai.sdk.image.duration", metrics.WithLabel("model", b.model)).Observe(duration.Seconds())
	}
...
func NewImageEditBuilder(ctx context.Context, generator ImageGenerator, logger logger.Logger, metrics metrics.Metrics) *ImageEditBuilder {
...
func NewImageVariationBuilder(ctx context.Context, generator ImageGenerator, logger logger.Logger, metrics metrics.Metrics) *ImageVariationBuilder {
