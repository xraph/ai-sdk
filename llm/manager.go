...
func (m *LLMManager) updateStats(providerName, operation string, latency time.Duration, err error, usage *LLMUsage) {
	...
	// Update metrics
	if m.metrics != nil {
		m.metrics.Counter("forge.ai.llm.requests_total", metrics.WithLabel("provider", providerName), metrics.WithLabel("operation", operation)).Inc()

		if err != nil {
			m.metrics.Counter("forge.ai.llm.requests_failed", metrics.WithLabel("provider", providerName), metrics.WithLabel("operation", operation)).Inc()
		} else {
			m.metrics.Counter("forge.ai.llm.requests_successful", metrics.WithLabel("provider", providerName), metrics.WithLabel("operation", operation)).Inc()
		}

		m.metrics.Histogram("forge.ai.llm.request_duration", metrics.WithLabel("provider", providerName), metrics.WithLabel("operation", operation)).Observe(latency.Seconds())

		if usage != nil {
			m.metrics.Counter("forge.ai.llm.tokens_total", metrics.WithLabel("provider", providerName)).Add(float64(usage.TotalTokens))
			m.metrics.Counter("forge.ai.llm.tokens_input", metrics.WithLabel("provider", providerName)).Add(float64(usage.InputTokens))
			m.metrics.Counter("forge.ai.llm.tokens_output", metrics.WithLabel("provider", providerName)).Add(float64(usage.OutputTokens))
		}
	}
}
