...
func (mm *MemoryManager) Store(ctx context.Context, content string, metadata map[string]any, importance float64) (*MemoryEntry, error) {
	...
	if mm.embedFn != nil {
		...
		if err != nil {
			if mm.logger != nil {
				mm.logger.Warn("Failed to generate embedding", logger.String("error", err.Error()))
			}
		} else {
	...
	if len(mm.working) > mm.workingCapacity {
		if err := mm.consolidateWorking(ctx); err != nil {
			if mm.logger != nil {
				mm.logger.Warn("Failed to consolidate working memory", logger.String("error", err.Error()))
			}
		}
	}

	if mm.metrics != nil {
		mm.metrics.Counter("forge.ai.sdk.memory.store", metrics.WithLabel("tier", string(MemoryTierWorking))).Inc()
	}
...
func (mm *MemoryManager) Recall(ctx context.Context, query string, tier MemoryTier, limit int) ([]*MemoryEntry, error) {
	...
	if mm.metrics != nil {
		mm.metrics.Counter("forge.ai.sdk.memory.recall", metrics.WithLabel("tier", string(tier))).Inc()
	...
	for i := 0; i < len(scored) && moved < toMove; i++ {
		...
		if mm.logger != nil {
			mm.logger.Debug("Consolidated memory to short-term",
				logger.String("memory_id", entry.ID),
				logger.Float64("importance", entry.Importance),
			)
		}
	}
