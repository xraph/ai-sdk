...
func NewPromptTemplateManager(logger logger.Logger, metrics metrics.Metrics) *PromptTemplateManager {
...
func (ptm *PromptTemplateManager) RegisterTemplate(tmpl *PromptTemplate) error {
	...
	if ptm.logger != nil {
		ptm.logger.Info("Prompt template registered",
			...
			logger.String("version", tmpl.Version),
		)
	}

	if ptm.metrics != nil {
		ptm.metrics.Counter("forge.ai.sdk.prompt_templates.registered",
			metrics.WithLabel("name", tmpl.Name),
			metrics.WithLabel("version", tmpl.Version),
		).Inc()
	}
...
func (ptm *PromptTemplateManager) Render(name, version string, vars map[string]any) (string, error) {
	...
	if abTest, exists := ptm.activeABTests[name]; exists && abTest.IsActive {
		...
		if err != nil {
			...
			if ptm.logger != nil {
				ptm.logger.Warn("A/B test variant selection failed, using default", logger.Error(err))
			}
		}
	}
...
func (ptm *PromptTemplateManager) UpdateTemplate(name string, newTemplate string) (string, error) {
	...
	if ptm.logger != nil {
		ptm.logger.Info("Template updated",
			...
			logger.String("old_version", latestVersion),
			logger.String("new_version", newVersion),
		)
	}
...
func (ptm *PromptTemplateManager) DeleteTemplate(name, version string) error {
	...
	if ptm.logger != nil {
		ptm.logger.Info("Template deleted", logger.String("name", name), logger.String("version", version))
	}
...
func (ptm *PromptTemplateManager) CreateABTest(test *ABTest) error {
	...
	if ptm.logger != nil {
		ptm.logger.Info("A/B test created",
			logger.String("name", test.Name),
			logger.Int("variants", len(test.Variants)),
		)
	}
		...
		if r <= cumulative {
			...
			if ptm.metrics != nil {
				ptm.metrics.Counter("forge.ai.sdk.prompt_templates.ab_test.variant_selected",
					metrics.WithLabel("test", test.Name),
					metrics.WithLabel("variant", variant.Name),
				).Inc()
			}
...
func (ptm *PromptTemplateManager) StopABTest(testName string) error {
	...
	if ptm.logger != nil {
		ptm.logger.Info("A/B test stopped", logger.String("name", testName))
	}
