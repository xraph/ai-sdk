...
func (r *RAG) IndexDocument(ctx context.Context, doc Document) error {
	...
	if r.logger != nil {
		r.logger.Debug("Indexing document",
			...
			logger.Int("content_length", len(doc.Content)),
		)
	}
	...
	if r.logger != nil {
		r.logger.Info("Document indexed",
			...
			logger.Duration("took", time.Since(startTime)),
		)
	}
...
func (r *RAG) Retrieve(ctx context.Context, query string) (*RetrievalResult, error) {
	...
	if err != nil {
		if r.metrics != nil {
			r.metrics.Counter("forge.ai.sdk.rag.retrieve.errors",
				metrics.WithLabel("error", err.Error()),
			).Inc()
		}
	...
	if r.logger != nil {
		r.logger.Info("Documents retrieved",
			logger.String("query", query),
			logger.Int("count", len(documents)),
			logger.Duration("took", result.Took),
		)
	}
