...
func NewCircuitBreaker(config CircuitBreakerConfig, logger logger.Logger, metrics metrics.Metrics) *CircuitBreaker {
...
func (cb *CircuitBreaker) Execute(ctx context.Context, fn func(context.Context) error) error {
	if !cb.canExecute() {
		if cb.metrics != nil {
			cb.metrics.Counter("forge.ai.sdk.circuit_breaker.rejected",
				metrics.WithLabel("circuit", cb.name),
			).Inc()
		}
...
func (cb *CircuitBreaker) canExecute() bool {
	...
	case CircuitStateOpen:
		...
		if time.Since(cb.lastFailureTime) > cb.resetTimeout {
			...
			if cb.logger != nil {
				cb.logger.Info("Circuit breaker transitioning to half-open", logger.String("circuit", cb.name))
			}
...
func (cb *CircuitBreaker) recordResult(err error) {
	...
	if err != nil {
		...
		if cb.state == CircuitStateHalfOpen {
			...
			if cb.logger != nil {
				cb.logger.Warn("Circuit breaker reopening after half-open failure", logger.String("circuit", cb.name))
			}
		...
		if cb.metrics != nil {
			cb.metrics.Counter("forge.ai.sdk.circuit_breaker.failures",
				metrics.WithLabel("circuit", cb.name),
			).Inc()
		}
	} else {
		...
		case CircuitStateHalfOpen:
			...
			if cb.halfOpenSuccess >= cb.halfOpenMax {
				...
				cb.failures = 0
				if cb.logger != nil {
					cb.logger.Info("Circuit breaker closed", logger.String("circuit", cb.name))
				}
			}
		case CircuitStateClosed:
		...
		if cb.metrics != nil {
			cb.metrics.Counter("forge.ai.sdk.circuit_breaker.successes",
				metrics.WithLabel("circuit", cb.name),
			).Inc()
		}
	}
...
func Retry(ctx context.Context, config RetryConfig, log logger.Logger, fn func(context.Context) error) error {
	var lastErr error
...
func NewRateLimiter(name string, rate, burst int, logger logger.Logger, metrics metrics.Metrics) *RateLimiter {
	return &RateLimiter{
		name:       name,
		rate:       rate,
		burst:      burst,
...
func (rl *RateLimiter) Allow() bool {
	...
	if rl.tokens >= 1 {
		rl.tokens--
		if rl.metrics != nil {
			rl.metrics.Counter("forge.ai.sdk.rate_limiter.allowed",
				metrics.WithLabel("limiter", rl.name),
			).Inc()
		}
	...
	if rl.metrics != nil {
		rl.metrics.Counter("forge.ai.sdk.rate_limiter.rejected",
			metrics.WithLabel("limiter", rl.name),
		).Inc()
	}
...
func NewFallbackChain(name string, logger logger.Logger, metrics metrics.Metrics) *FallbackChain {
...
func (fc *FallbackChain) Execute(ctx context.Context, fns ...func(context.Context) error) error {
	...
	for i, fn := range fns {
		...
		if err == nil {
			if fc.metrics != nil {
				fc.metrics.Counter("forge.ai.sdk.fallback.success",
					metrics.WithLabel("chain", fc.name),
					metrics.WithLabel("attempt", strconv.Itoa(i+1)),
				).Inc()
			}
		...
		if fc.logger != nil {
			fc.logger.Warn("Fallback attempt failed",
				logger.String("chain", fc.name),
				logger.Int("attempt", i+1),
				logger.Int("total", len(fns)),
				logger.Error(err),
			)
		}
	}
	...
	if fc.metrics != nil {
		fc.metrics.Counter("forge.ai.sdk.fallback.exhausted",
			metrics.WithLabel("chain", fc.name),
		).Inc()
	}
...
func NewBulkhead(name string, maxConcurrent int, logger logger.Logger, metrics metrics.Metrics) *Bulkhead {
	return &Bulkhead{
		name:          name,
...
func (b *Bulkhead) Execute(ctx context.Context, fn func(context.Context) error) error {
	...
	case b.semaphore <- struct{}{}:
		...
		if b.metrics != nil {
			b.metrics.Counter("forge.ai.sdk.bulkhead.accepted",
				metrics.WithLabel("bulkhead", b.name),
			).Inc()
			b.metrics.Gauge("forge.ai.sdk.bulkhead.active",
				metrics.WithLabel("bulkhead", b.name),
			).Set(float64(len(b.semaphore)))
		}

		return fn(ctx)
	case <-ctx.Done():
		if b.metrics != nil {
			b.metrics.Counter("forge.ai.sdk.bulkhead.timeout",
				metrics.WithLabel("bulkhead", b.name),
			).Inc()
		}
...
func Timeout(ctx context.Context, timeout time.Duration, logger logger.Logger, metrics metrics.Metrics, fn func(context.Context) error) error {
