...
func TestTimeout_Success(t *testing.T) {
	err := Timeout(context.Background(), 100*time.Millisecond, testhelpers.NewMockLogger(), testhelpers.NewMockMetrics(), func(ctx context.Context) error {
		return nil
	})
	if err != nil {
		t.Errorf("expected no error, got %v", err)
...
func TestTimeout_Exceeded(t *testing.T) {
	err := Timeout(context.Background(), 50*time.Millisecond, testhelpers.NewMockLogger(), testhelpers.NewMockMetrics(), func(ctx context.Context) error {
