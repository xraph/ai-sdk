...
func (s *UIPartStreamer) Start() error {
	...
	if s.metrics != nil {
		s.metrics.Counter("forge.ai.sdk.ui_stream.started", metrics.WithLabel("part_type", string(s.partType))).Inc()
	}
...
func (s *UIPartStreamer) End() error {
	...
	if s.metrics != nil {
		s.metrics.Counter("forge.ai.sdk.ui_stream.completed",
			metrics.WithLabel("part_type", string(s.partType)),
		).Inc()
...
func NewUIPartStreamManager(onEvent func(llm.ClientStreamEvent) error, logger logger.Logger, metrics metrics.Metrics) *UIPartStreamManager {
