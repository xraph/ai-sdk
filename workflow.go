...
func NewWorkflow(id, name string, logger logger.Logger, metrics metrics.Metrics) *Workflow {
	return &Workflow{
		ID:         id,
		Name:       name,
...
func (w *Workflow) Execute(ctx context.Context, input map[string]any) (*WorkflowExecution, error) {
	...
	if w.metrics != nil {
		w.metrics.Counter("forge.ai.sdk.workflow.executions",
			metrics.WithLabel("workflow", w.ID),
...
func (w *Workflow) executeToolNode(ctx context.Context, node *WorkflowNode, execution *WorkflowExecution) (any, error) {
	...
	if w.metrics != nil {
		w.metrics.Counter("forge.ai.sdk.workflow.tool_executions",
			metrics.WithLabel("workflow", w.ID),
			metrics.WithLabel("tool", node.ToolName),
		).Inc()
	}
...
func (w *Workflow) executeAgentNode(ctx context.Context, node *WorkflowNode, execution *WorkflowExecution) (any, error) {
	...
	if w.metrics != nil {
		w.metrics.Counter("forge.ai.sdk.workflow.agent_executions",
			metrics.WithLabel("workflow", w.ID),
			metrics.WithLabel("agent", node.AgentID),
		).Inc()
	}
